...
var ManagePatientComponent = (function () {
    ...
    ManagePatientComponent.prototype.getAllPaginatedPatientFromServer = function (page) {
        var _this = this;
        if (page > 0) {
            page = page;
        }
        this.requestsService.getRequest(app_constants_1.AppConstants.FETCH_ALL_PATIENT_URL + page)
        ...
        if (localStorage.getItem(btoa('access_token'))) {
            ...
            this.requestsService.deleteRequest(app_constants_1.AppConstants.PATIENT_DELETE_URI + patientId)
                ...
                if (response['responseCode'] === 'PATIENT_SUC_06') {
                    ...
                    _this.getAllPaginatedPatientFromServer(0);
                }
                else {
                    _this.getAllPaginatedPatientFromServer(0);
    ...
    ManagePatientComponent.prototype.searchPatient = function (page) {
        var _this = this;
        this.searched = true;
        if (page > 0) {
            page = page;
        }
        this.requestsService.getRequest(app_constants_1.AppConstants.SEARCH_ALL_PATIENT_URL + page + '?searchString=' + this.searchString)
            .subscribe(function (response) {
            if (response['responseCode'] === 'PATIENT_SUC_11') {
                _this.nextPage = response['responseData']['nextPage'];
                _this.prePage = response['responseData']['prePage'];
