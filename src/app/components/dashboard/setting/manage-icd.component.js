...
var ManageICDComponent = (function () {
    function ManageICDComponent(notificationService, requestsService, HISUtilService, router) {
        ...
        this.data = [];
        this.searchVersion = "";
        this.selectedICDVersionUpdate = "";
    }
    ManageICDComponent.prototype.ngOnInit = function () {
        document.title = 'HIS | Manage ICD';
        if (window.localStorage.getItem(btoa('access_token'))) {
            this.getICDCVsFromServer(0);
        }
    };
    ManageICDComponent.prototype.versionsPopupLoadByServer = function () {
        var _this = this;
        if (window.localStorage.getItem(btoa('access_token'))) {
    ...
    ManageICDComponent.prototype.codesPopupLoadByServer = function () {
        var _this = this;
        if (window.localStorage.getItem(btoa('access_token'))) {
    ...
    ManageICDComponent.prototype.onAddICDCVMPopupLoad = function () {
        this.versionsPopupLoadByServer();
        this.codesPopupLoadByServer();
        //this.versionChanged(this.iCDCVM.selectedICDVersionId, 'chk-');
    };
    ManageICDComponent.prototype.getPageWiseICDs = function (page) {
        this.data = [];
        if (this.searched) {
            this.searchCodeVersionByVersionName(page);
            // this.getSearchCVsByVersionNameFromServer('/setting/icd/codeVersion/search/', page);
        }
        else {
            this.getICDCVsFromServer(page);
        }
    };
    ManageICDComponent.prototype.refreshCodeVersionTable = function () {
        this.searched = false;
        this.getICDCVsFromServer(0);
    };
    ManageICDComponent.prototype.refreshICDsTable = function (page) {
        this.getICDCVsFromServer(page);
    };
    ManageICDComponent.prototype.deleteCodeVersion = function (associateICDCVId) {
        var _this = this;
        if (window.localStorage.getItem(btoa('access_token'))) {
    ...
    ManageICDComponent.prototype.getICDCVsFromServer = function (page) {
        var _this = this;
        if (page > 0) {
            page = page;
        }
        this.requestsService.getRequest('/setting/icd/iCDCVs/' + page)
            .subscribe(function (response) {
    ...
    ManageICDComponent.prototype.saveCodeVersion = function () {
        var _this = this;
        /* let checkedFound;
         checkedFound = false;*/
        ...
        // for (let checkedCode of  this.iCDCodes) {
        //     if (checkedCode.checkedCode) {
        //         checkedFound = true;
        //         break;
        //     }
        // }
        // if (!checkedFound) {
        //     this.notificationService.error('Please select Code ', 'Association');
        //     return;
        // }
    ...
    ManageICDComponent.prototype.editCV = function (itemCVM) {
        this.versionsPopupLoadByServer();
        this.selectedICDVersionUpdate = itemCVM.icdVersion.name;
        this.iCDCVM.selectedICDVersionId = itemCVM.icdVersion.id;
        this.codesPopupLoadByServer();
        this.versionChanged(itemCVM.icdVersion.id, 'chk-cv');
    };
    ManageICDComponent.prototype.searchCodeVersionByVersionName = function (page) {
        var _this = this;
        if (window.localStorage.getItem(btoa('access_token'))) {
            this.searched = true;
    ...
    ManageICDComponent.prototype.versionChanged = function (associatedICDCVId, checkBoxId) {
        var _this = this;
        ...
        if (window.localStorage.getItem(btoa('access_token'))) {
            console.log('now getting all already associated against given Id of Selected ICD Version');
            this.requestsService.getRequest('/setting/icd/getAssociatedByVId/?associatedICDCVId=' + associatedICDCVId)
                .subscribe(function (response) {
    ...
    ManageICDComponent = __decorate([
        core_1.Component({
            selector: 'manage-icd-component',
            templateUrl: '../../../templates/dashboard/setting/manage-icd.template.html',
        }),
    ...
    return ManageICDComponent;
}());
exports.ManageICDComponent = ManageICDComponent;
//# sourceMappingURL=manage-icd.component.js.map