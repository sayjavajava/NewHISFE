import { ClinicalDepartment } from './../../../../../public/src/app/models/clinical-department';
...
export class UpdatedoctorComponent implements OnInit {
            ...
            .subscribe(
                (response: Response) => {
                    if (response['responseCode'] === 'CLI_DPT_SUC_01') {
                        ...
                        this.departmentList2=data['name'];
    ...
    addFields(): void {
        this.departmentList2 = this.userForm.get('departmentList2') as FormArray;
        ...
        if (this.id) {
            this.requestService.findById(AppConstants.FETCH_USER_BY_ID + this.id).subscribe(
                user => {
                   ...
                   this.userForm.controls['departmentList2'].patchValue(user.ClinicalDepartment);
    ...
    addUser(data: any) {
        ...
        if (this.userForm.valid) {
            ...
            let doctor = new User({
                ...
                email: data.email,
                selectedVisitBranches: data.selectedVisitBranches,
    ...
    selectRestrictBranch(event: any, item: any) {
        console.log(item);
        if (event.target.checked) {
            this.selectedVisitBranches.push(item);
        }
        else {
            let updateItem = this.selectedVisitBranches.find(this.findIndexToUpdate, item.name);
