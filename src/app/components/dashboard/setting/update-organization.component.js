    ...
    UpdateOrganizationComponent.prototype.getStatesById = function (id) {
        ...
        this.requestService.getRequest(app_constants_1.AppConstants.GET_STATE_BYCOUNTRYID + id)
            ...
            if (response["responseCode"] === "STATE_SUC_11") {
                _this.stateLst = response['responseData'].statesList;
                console.log(_this.stateLst);
                if (_this.stateLst.length > 0) {
                    ...
                    _this.citiesListModified = [];
                    _this.selectedCity = [];
                    _this.citiesListModified.push({ label: 'Not Applicable', value: '-1' });
                    _this.selectedCity.push({ label: 'Not Applicable', value: '-1' });
